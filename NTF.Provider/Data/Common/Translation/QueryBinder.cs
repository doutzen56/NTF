...
namespace NTF.Provider.Data.Common
{
    ...
    public class QueryBinder : DbExpressionVisitor
    {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            else if (typeof(NonQuery).IsAssignableFrom(m.Method.DeclaringType)) 
            {
                IDbContext upd = this.batchUpd != null
                    ? this.batchUpd
                    : (IDbContext)((ConstantExpression)m.Arguments[0]).Value;
        ...
        private Expression BindInsert(IDbContext upd, Expression instance, LambdaExpression selector)
        {
        ...
        protected override Expression VisitConstant(ConstantExpression c)
        {
            if (this.IsQuery(c))
            {
                ...
                IDbContext t = q as IDbContext;
                if (t != null)
                {
                    IHaveMappingEntity ihme = t as IHaveMappingEntity;
                    MappingEntity entity = ihme != null ? ihme.Entity : this.mapper.Mapping.GetEntity(t.ElementType, t.TableName);
