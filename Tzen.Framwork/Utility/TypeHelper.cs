...
namespace Tzen.Framwork.Utility
{
    public static class TypeHelper
    {
        public static Type FindIEnumerable(Type sourType)
        {
            if (sourType == null || sourType == typeof(string))
                return null;
            if (sourType.IsArray)
                return typeof(IEnumerable<>).MakeGenericType(sourType.GetElementType());
            if (sourType.IsGenericType)
            {
                foreach (Type arg in sourType.GetGenericArguments())
                {
                    Type ienum = typeof(IEnumerable<>).MakeGenericType(arg);
                    if (ienum.IsAssignableFrom(sourType))
                    {
                        return ienum;
                    }
                }
            }
            Type[] ifaces = sourType.GetInterfaces();
            ...
            if (sourType.BaseType != null && sourType.BaseType != typeof(object))
            {
                return FindIEnumerable(sourType.BaseType);
            }
            return null;
        }
